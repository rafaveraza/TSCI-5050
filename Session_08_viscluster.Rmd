...
```{r}
rawdeid.sam$VAR01<-rnorm(nrow(toclust.sam),mean = 5,sd=2.5)
rawdeid.sam$VAR02<-rlnorm(nrow(toclust.sam),mean = 2.5,sd=1.2)
rawdeid.sam$VAR03<-rchisq(nrow(toclust.sam),ncp = 3)
```

### Correlation Matrix
Which variables correlate with which other variables? Let's create a correlation matrix. 
```{r}
 rawdeid.cor <- cor(rawdeid.sam,use = 'pairwise');
```
...
### K-Means Clustering
The idea behind K-means is to optimally divide the data into K clusters. First, we empirically determine how many clusters the data "wants" to be divided into.
